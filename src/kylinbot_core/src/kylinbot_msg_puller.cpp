...
void publishOdomeMsg(ros::Publisher *pub, const KylinMsg_t* kylinMsg)
{
  ...
  float py = -kylinMsg->cbus.cp.x / KYLIN_MSG_VALUE_SCALE;
  float px = kylinMsg->cbus.cp.y / KYLIN_MSG_VALUE_SCALE;
  float pz = -kylinMsg->cbus.cp.z / KYLIN_MSG_VALUE_SCALE; // rad to deg
  float vy = -kylinMsg->cbus.cv.x / KYLIN_MSG_VALUE_SCALE;
  float vx = kylinMsg->cbus.cv.y / KYLIN_MSG_VALUE_SCALE;
  float vz = -kylinMsg->cbus.cv.z / KYLIN_MSG_VALUE_SCALE; // rad/s to
...
void publishOdomeMsg(ros::Publisher *pub, const KylinMsg_t* kylinMsg)
{
  //float py = -kylinMsg->cbus.cp.x / KYLIN_MSG_VALUE_SCALE;
  //float px = kylinMsg->cbus.cp.y / KYLIN_MSG_VALUE_SCALE;
  //float pz = -kylinMsg->cbus.cp.z / KYLIN_MSG_VALUE_SCALE;
  // cvy = -kylinMsg->cbus.cv.x / KYLIN_MSG_VALUE_SCALE;
  //float cvx = kylinMsg->cbus.cv.y / KYLIN_MSG_VALUE_SCALE;
  //float cvz = -kylinMsg->cbus.cv.z / KYLIN_MSG_VALUE_SCALE;
  
  float cvy = -kylinMsg->cbus.cv.x / KYLIN_MSG_VALUE_SCALE;
  float cvx = kylinMsg->cbus.cv.y / KYLIN_MSG_VALUE_SCALE;
  float cvz = -kylinMsg->cbus.cv.z / KYLIN_MSG_VALUE_SCALE;
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher odo_pub = n.advertise<kylinbot_core::Kylin>("odo", 100); // Odometry feedback
  ros::Publisher rcf_pub = n.advertise<kylinbot_core::VirtualRC>("rcf", 100); // Remote controller feedback
  ros::Publisher sonar_pub = n.advertise<kylinbot_core::Sonar>("sonar", 100); // Sonar feedback
  ros::Publisher zgyro_pub = n.advertise<kylinbot_core::ZGyro>("zgyro", 100); // ZGyro feedback
  ros::Publisher calib_pub = n.advertise<kylinbot_core::PosCalib>("calib", 100); // Calib feedback
  
  ros::Publisher odom_pub = n.advertise<nav_msgs::Odometry>("odom", 100); // Built-in odom
  
  ros::Publisher uwb_pub = n.advertise<kylinbot_core::UWB>("uwb", 100); // Built-in odom
  ...
  while (ros::ok())
  {
    ...
    if (Msg_Pop(&rx_fifo, rx_buf[1], &msg_head_uwb, &uwbMsg)) {
